...
public class Building_Creator : MonoBehaviour
{
    ...
    public void EdgeCreator(int i, int x, int y, int[] height, Vector2Int gridDimensions, Grid_Generator.Sector[,] sectorGrid, int sectionSideLength, int streetWidth)
    {
        ...
        else if (sectorGrid[x, y].connected[left] && sectorGrid[x, y].connected[front])
        {
            ...
            {
                Instantiate(cornerCap, new Vector3(xMod, (height[id] + 1) * sectionHeight,  yMod), Quaternion.Euler(-90, 0, direction), GameObject.Find("Tower (" + id + ")").transform).name = ("Top Section  | Corner Cap | Coords: " + x + ", " + y);
                return;
            }
            else
        ...
        Instantiate    (edgeStack[0], new Vector3(xMod, 0,                                  yMod), Quaternion.Euler(-90, 0, direction), GameObject.Find("Tower (" + id + ")").transform).name = ("Bottom Section | " + edgeStack[0].name + " | Coords: " + x + ", " + y);
        for (int b = 1; b <= height[id]; b++)
        {
            Instantiate(edgeStack[1], new Vector3(xMod, b * sectionHeight,                  yMod), Quaternion.Euler(-90, 0, direction), GameObject.Find("Tower (" + id + ")").transform).name = ("Middle Section (" + b + ")  | " + edgeStack[0].name + " | Coords: " + x + ", " + y);
        }
        Instantiate    (edgeStack[2], new Vector3(xMod, (height[id] + 1) * sectionHeight,   yMod), Quaternion.Euler(-90, 0, direction), GameObject.Find("Tower (" + id + ")").transform).name = ("Top Section  | " + edgeStack[0].name + " | Coords: " + x + ", " + y);
    }
}
